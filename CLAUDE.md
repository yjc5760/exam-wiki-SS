...
請 Cowork 更新 question_index.json（tags、verified）
    ↓
Claude Code: ingest SS-XXXX-N → wiki 自動更新
```
...
| **Cowork** | 解題（SOLVE，**一次一題**）、存檔（.md + viz.html）、更新 question_index.json（hasSolution/tags/designMethod）、考題分布分析、直接維護 wiki/diagnosis/ · wiki/failure-modes/ · wiki/materials/ · wiki/code-ref/ · wiki/queries/ |
| **Claude Code** | ingest、compile-all、lint、add method（詳見 CLAUDE-CODE.md） |
| **Chat（claude.ai）** | 討論架構、規劃設計（已不常用，現多由 Cowork 取代） |
...
```
raw/solutions/SS-XXXX-N/SS-XXXX-N.md  ──→  wiki/problems/      （Claude Code: ingest）
raw/json/concepts.json                 ──→  wiki/concepts/      （Claude Code: compile）
raw/solutions/methods/                 ──→  wiki/methods/       （Claude Code: add method）
...
└── wiki/                            ← 知識庫輸出
    ...
    ├── concepts/                    ← 概念頁         ← Claude Code
    ├── methods/                     ← 方法論頁       ← Claude Code
    ├── traps/                       ← 陷阱頁         ← Claude Code
    ├── problems/                    ← 題目頁         ← Claude Code
    ├── philosophy/                  ← 設計哲學頁     ← Claude Code
...
Wiki 導航依七層知識架構組織（前三層由 Claude Code 生成，後四層由 Cowork 直接維護）：

| 層 | 目錄 | 維護者 | 內容 |
|----|------|:------:|------|
| Layer 1 | `concepts/` + `problems/` | Claude Code | 核心構件設計（拉壓/梁/梁柱/接合） |
| Layer 2 | `philosophy/` | Claude Code | 設計哲學與實務（耐震/材料/施工） |
| Layer 3 | `methods/` | Claude Code | 解題方法論（共軛梁/虛功/P-M圖） |
...
8. **格式與命名規範見 CLAUDE-SPEC.md；Claude Code 指令見 CLAUDE-CODE.md**
